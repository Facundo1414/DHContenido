...
function renderizarAlbumes(listado) {
    ...
    // se actualiza los albums y favoritos
...
function mostrarDatosEnPerfil(albumes) {
    ...
    document.querySelector("#cant-albums").innerHTML = 
    `${albumes.length == 1 ? "1 album" : `${albumes.length} albumes`}`;
    // for each y luego operador Ternario
    let total = 0;
    albumes.forEach(album => {
        album.like ? total++ : "" 
    })
    document.querySelector("#cant-favoritos").innerHTML = 
    `${total == 1 ? `${total} favorito` : `${total} favoritos`}`;;
