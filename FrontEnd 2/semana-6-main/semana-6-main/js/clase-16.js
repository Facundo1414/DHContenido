...
//    Esta funcion retorna una promesa, por eso capturamos su resultado con el then()
   ...
   .then( respuesta => renderizarElementosv1(respuesta))
})
...
function renderizarElementosv2(listado) {
    const cajaComentarios = document.querySelector(".comentarios");
    
    const comentariosHTML = listado.map(element => {
    return `<div class="comentario">
        <h4>${element.email}</h4>
        <p>${element.body}</p> </div>`;
    });
    cajaComentarios.innerHTML += comentariosHTML.join('');
}

function renderizarElementosv3(listado) {
    const cajaComentarios = document.querySelector(".comentarios");
    ...
    listado.map(element => {
      ...
      emailH4.textContent = element.email;
      bodyP.textContent = element.body;
      
      comentarioDiv.classList.add("comentario");
      comentarioDiv.appendChild(emailH4);
      comentarioDiv.appendChild(bodyP);
      
      fragmento.appendChild(comentarioDiv);
    });
    
    cajaComentarios.appendChild(fragmento);
  }
